...
# Define a data atual (errado: .today() não aceita parâmetros - precisa de correção)
hoje = datetime.datetime.today('YYYY-MM-DD')  # <-- Isso vai gerar erro se não for corrigido

# Define o caminho do arquivo CSV de saída
arq = rf'relatorio1/relatorio_acionamento-{hoje}.csv'

# Cria o cabeçalho do arquivo CSV
file = open(arq, 'a')
dt_inicio = datetime.date(2023,3,6)
...
# Laço para percorrer os dias entre dt_inicio e hoje
while dt_inicio < datetime.date.today():
    file = open(arq, 'a')

    # Consulta SQL com várias CTEs para consolidar estatísticas de discagens
    command = f"""
WITH
...
base_cpf AS (SELECT idscript, count( DISTINCT cpf) AS cpf FROM base_geral GROUP BY idscript),
...
relatorio AS (SELECT a.idscript AS idscript, a.tentativas AS tentativas, b.atendimentos AS atendimentos, c.FINAL AS FINAL, 
...
LEFT JOIN base_badphone d ON a.idscript = d.idscript LEFT JOIN base_telefonia e ON a.idscript = e.idscript
LEFT JOIN base_machine f ON a.idscript = f.idscript LEFT JOIN base_block g ON a.idscript = g.idscript 
LEFT JOIN base_cpf i ON a.idscript = i.idscript LEFT JOIN base_telefones J ON a.idscript = j.idscript
LEFT JOIN base_custo1 k ON a.idscript = k.idscript),


relatorio_v2 AS (SELECT idscript, tentativas, cpf, CASE WHEN atendimentos IS NULL THEN 0 ELSE atendimentos END AS atendimentos, custo, telefones,
CASE WHEN FINAL IS NULL THEN 0 ELSE FINAL END AS FINAL, badphone, telefonia, machine, block FROM RELATORIO),


relatorio_v3 AS (SELECT idscript, tentativas, cpf, (atendimentos + final) AS atendimentos, FINAL, telefones, badphone, telefonia, machine, block, custo
FROM relatorio_v2),
...
relatorio_v4 AS ( SELECT substr(b.DATA, 1, 10) AS DATA, '' AS cliente, a.idscript AS campanha_idscript, b.script AS script, '' AS canais_telefonia, '' AS segmentos_base, '' qtd_base, '' AS canais_acao, '' AS periodo_exec_de, '' AS periodo_exec_ate,
...
SELECT * FROM relatorio_v4 """
    ...
    for iten in dados:
        # Formata os dados em CSV e remove caracteres desnecessários
        iten = '\n' + str(iten)
